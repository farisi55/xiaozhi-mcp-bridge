...
│   └── tools/
│       └── index.ts        # Definisi & implementasi tools (cek_jawaban_angka, catat_ringkasan_sesi)
├── .github/workflows/
...
4. Uji tool call: tanyakan sesuatu ke robot yang memicu tool (misalnya soal kuis angka), lalu cek log (`npm run tail`) apakah baris `tools/call` muncul dan tidak error.

5. Cek ringkasan sesi tersimpan:
   ```bash
   curl https://xiaozhi-mcp-bridge.<subdomain-kamu>.workers.dev/sessions
   ```
